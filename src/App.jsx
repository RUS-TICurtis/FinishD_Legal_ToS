...
function App() {
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface selection:bg-primary-container selection:text-white mesh-gradient">
      {/* Header */}
      <header className={`fixed top-0 w-full z-50 transition-all duration-300 border-b ${scrolled ? 'bg-black/80 backdrop-blur-xl border-white/10' : 'bg-transparent border-transparent'}`}>
      ...
      <main className="pt-24">
        ...
        {/* FAQ Section */}
        <section className="px-8 py-24 bg-soft-grey">
          <div className="max-w-[1000px] mx-auto">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {UI_CONTENT.homeFaqs.map((faq, i) => (
                <div key={i} className="faq-glass-card p-8 rounded-[2.5rem] border border-deep-black/5 hover:border-vibrant-green/30 transition-all group">
                  <h4 className="text-xl font-bold text-deep-black mb-4 group-hover:text-vibrant-green transition-colors">{faq.q}</h4>
                  <p className="text-deep-black/70 leading-relaxed">{faq.a}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Footer */}
      <footer className="w-full border-t border-white/10 pt-16 pb-8 bg-black">
        <div className="max-w-[1440px] mx-auto px-8 flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex flex-wrap justify-center gap-8">
            <button onClick={() => openModal('privacy')} className="text-sm text-white/40 hover:text-mint-400">Privacy</button>
            <button onClick={() => openModal('terms')} className="text-sm text-white/40 hover:text-mint-400">Terms of Use</button>
            <button onClick={() => openModal('guidelines')} className="text-sm text-white/40 hover:text-mint-400">Guidelines</button>
                  ...
                  <div className="space-y-12">
                    ...
                    <div className="space-y-6">
                      ...
                      {UI_CONTENT.help.faqs.map(faq => (
                        <div key={faq.id} className="faq-glass-card rounded-2xl border border-deep-black/5 p-8 hover:border-vibrant-green/30 transition-colors">
                          <h3 className="text-xl font-bold text-deep-black mb-4">{faq.q}</h3>
                          <p className="text-deep-black/70 text-lg leading-relaxed">{faq.a}</p>
                        </div>
                      ))}
                    </div>
                  </div>
    ...
    <div className="max-w-3xl mx-auto space-y-12">
      ...
      <form onSubmit={handleSubmit} className="space-y-8 glass-card p-8 md:p-12 rounded-[3rem]">
        ...
        <button
          ...
          className="w-full bg-primary-container text-white font-black py-5 rounded-2xl hover:opacity-90 transition-all text-lg uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-3"
        >
          {status === 'loading' ? (
            <>
